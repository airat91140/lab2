...
X16::X16(char *num) {
    ...
    for (int i = strlen(num) - 1, j = 1; i >= 0; i -= 2) { //идем в цикле с конца строки, j нужно для контроляв какой байт записывать
        char tmp[2];
...
X16 X16::add(X16 sec) {
    ...
    dopoln(res, N, res2); //перводим из дополнительного кода обратно в прямой
    X16 result;
    result.setmas(res2);
    result.correctlen();
...
void dopoln(const unsigned char num[N], int len, unsigned char dop[N]) {
    for (int i = 0; i < len; ++i) //инициализируем дополнительеый код числа нулями
        dop[i] = 0;
