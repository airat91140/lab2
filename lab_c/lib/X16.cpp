...
ostream &operator <<(ostream &out, const X16 &num) {
    if ((N * 2 - num.len) & 1) //выписываем первые буквы в зависмости от того находятся они в первой или второй половине
        ...
        out << hex << (unsigned int) ((num.number[(N * 2 - num.len) / 2] >> 4) & 0x0f)
            << (unsigned int) (num.number[(N * 2 - num.len) / 2] & 0x0f);
    for (int i = (N * 2 - num.len) / 2 + 1; i < N; ++i) {//выписываем по очереди оставшиеся символы
